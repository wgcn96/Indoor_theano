...
configFileURL = '/home/ubuntu/wangchen/work/Indoor/config/0208train_1.conf'
...
reload_image_url = '/home/ubuntu/wangchen/work/Indoor/models/caffe_vgg16_image.pkl'
reload_places_url = '/home/ubuntu/wangchen/work/Indoor/models/caffe_vgg16_places.pkl'
...
net_final_out,net_global_out,net_inc1_out = buildSpatialTransformerNet_one()
...
count3 = copyParams(lasagneLayersList[44:65],copyImageParamsList)
...
print '\tcopy params are: {} {} {} '.format(count1,count2,count3)
wangchendebug += 1
...
# training output
output_train,global_output,inc1_output = lasagne.layers.get_output([net_final_out,net_global_out,net_inc1_out], X, deterministic=False)
...
updates = lasagne.updates.momentum(cost,lasagneParamsList[:30],sh_lr)
updates.update(lasagne.updates.momentum(cost,lasagneParamsList[30:60],stn_lr_mul*sh_lr))
updates.update(lasagne.updates.momentum(cost,lasagneParamsList[60:],sh_lr))


train = theano.function([X,y],[cost, output_train,global_cost,inc1_cost], updates=updates)
...
def train_epoch(X, y):
    ...
    for i in range(num_batches):
        ...
        X_batch = X[idx]
        y_batch = y[idx]
        cost_batch, output_train,global_cost_batch,inc1_cost_batch = train(X_batch, y_batch)
        costs += [cost_batch]
        global_cost += [global_cost_batch]
        inc1_cost += [inc1_cost_batch]
        preds = np.argmax(output_train, axis=-1)
        correct += np.sum(y_batch == preds)
    return np.mean(costs), correct / float(num_samples),np.mean(global_cost),np.mean(inc1_cost)
...
try:
    ...
    for n in range(num_epoches):
        ...
        train_cost, train_acc, global_cost, inc1_cost = train_epoch(MyDict['X_train'], MyDict['y_train'])
