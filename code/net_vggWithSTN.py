...
def buildSTN(stn_downsample_factor=1):
    ...
    googleNetDict_st = build_vgg_model(inputLayer=net_input, prefix=st_prefix,stnFlag=True, classificationFlag=False)
    ...
    net_stn_1 = lasagne.layers.TransformerLayer(net_input, net_st_theta_1_fc,mode=6, name=st_prefix+'stn_1',downsample_factor=stn_downsample_factor)

    net_stn_2 = lasagne.layers.TransformerLayer(net_input, net_st_theta_2_fc,mode=6, name=st_prefix+'stn_2',downsample_factor=stn_downsample_factor)

    #print "Transformer network output shape: ", net_stn_1.output_shape
    return net_stn_1,net_stn_2

def buildSTN_one(stn_downsample_factor=1):

    st_prefix = 'st/'

    b1 = [0.25 , 0, 0,  0, 0.25, 0]
    #b2 = [0.3 , 0, 0 , 0, 0.3, 0]
    b1 = np.asarray(b1,dtype='float32')
    #b2 = np.asarray(b2,dtype='float32')
...
def buildSpatialTransformerNet():
    ...
    net_inc1_Dict = build_vgg_model(inputLayer=net_stn_1,prefix=inc1_prefix,dropout_ratio=0.5,stnFlag=False,classificationFlag=True)
    net_inc2_Dict = build_vgg_model(inputLayer=net_stn_2,prefix=inc2_prefix,dropout_ratio=0.5,stnFlag=False,classificationFlag=True)
    net_global_Dict = build_vgg_model(inputLayer=net_input, prefix=global_prefix, dropout_ratio=0.5, stnFlag=False,classificationFlag=True)
...
def buildSpatialTransformerNet_one():
    ...
    net_final_concat = ConcatLayer([net_global_output, net_inc1_output], name='final/concat/output')
    ...
    return net_final_prob,net_global_prob,net_inc1_prob
