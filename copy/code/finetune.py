...
#global Variant
configFileURL = '/home/ubuntu/wangchen/work/Indoor/config/1220train_1.conf'
...
# model_url = '/media/wangchen/newdata1/wangchen/work/Indoor/models/best_model.pkl'
# resultFileURL = '/media/wangchen/newdata1/wangchen/work/Indoor/result/1219result_1.txt'
...
net_vgg_Dict = build_vgg_model(inputLayer,net_prefix,classificationFlag=True)
net_vgg_last_layer = net_vgg_Dict[net_prefix + 'prob']
lasagneLayerList = lasagne.layers.get_all_layers(net_vgg_last_layer)
lasagneParamList = lasagne.layers.get_all_params(net_vgg_last_layer, trainable=True)

lasagne.layers.set_all_param_values(net_vgg_last_layer, copyParamsList)
lasagneParamvaluesList = lasagne.layers.get_all_param_values(net_vgg_last_layer, trainable=True)
...
# training output
output_train = lasagne.layers.get_output([net_vgg_last_layer], X, deterministic=False)
output_train = output_train[0]
# evaluation output. Also includes output of transform for plotting
output_eval = lasagne.layers.get_output([net_vgg_last_layer], X, deterministic=True)
output_eval = output_eval[0]
sh_lr = theano.shared(lasagne.utils.floatX(learning_rate))
l2_penalty = regularize_network_params(net_vgg_last_layer,l2)
...
'''
updates = lasagne.updates.momentum(cost,lasagneParamList,sh_lr,momentum=0.9)
...
loc_params = new_params = lasagne.layers.get_all_param_values(net_vgg_last_layer,trainable=True)
try:
    max_test_acc = 0.
    idx = 0
    ...
    for n in range(num_epoches):
        ...
        new_params = lasagne.layers.get_all_param_values(net_vgg_last_layer,trainable=True)
        if n >= 1:
        ...
        if test_acc > max_test_acc:
            ...
            idx = n+1
            saveModel(model_url,lasagne.layers.get_all_param_values(net_vgg_last_layer,trainable = True))
